...
if not cap.isOpened() :
	print "error, could not find video file, please check filename"
...
try :
    while(cap.isOpened()):
        ...
        if DEBUG :
            cv2.imshow('background model',bgm.model.astype(np.uint8))
            cv2.imshow('mask',fgmask)
            if im_with_keypoints is not None :
                cv2.rectangle(im_with_keypoints, (hive.x, hive.y), (hive.x+hive.w, hive.y+hive.h), (255,0,0), 2)
                cv2.imshow("Keypoints", im_with_keypoints)
            cv2.imshow('input frame',frame)
            # wait for the key 'q' and quit
